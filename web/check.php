...
echo "********************************<BR>";
echo "*                              *<BR>";
echo "*  Symfony requirements check  *<BR>";
echo "*                              *<BR>";
echo "********************************<BR><BR>";
echo sprintf("php.ini used by PHP: %s<BR><BR>", $iniPath);

echo "** WARNING **<BR>";
echo "*  The PHP CLI can use a different php.ini file<BR>";
echo "*  than the one used with your web server.<BR>";
if ('\\' == DIRECTORY_SEPARATOR) {
    echo "*  (especially on the Windows platform)<BR>";
}
echo "*  If this is the case, please ALSO launch this<BR>";
echo "*  utility from your web server.<BR>";
echo "** WARNING **<BR>";
...
{
    echo $boolean ? "  OK        " : sprintf("<BR><BR>[[%s]] ", $fatal ? ' ERROR ' : 'WARNING');
    echo sprintf("$message%s<BR>", $boolean ? '' : ': FAILED');

    if (!$boolean) {
        echo "            *** $help ***<BR>";
        if ($fatal) {
            exit("You must fix this problem before resuming the check.<BR>");
        }
    }
}
